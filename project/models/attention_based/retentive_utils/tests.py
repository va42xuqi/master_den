...
class TestRetention(unittest.TestCase):
    ...
    def test_simple(self):
        ...
        for i in range(sequence_length // chunk_size):
            y_i, r_i = sr.forward_chunkwise(
                X[:, i * chunk_size : (i + 1) * chunk_size, :], r_n_1, i
            )
    ...
    def test_multiscale(self):
        ...
        print(
            "Default v_dim:",
            sum(p.numel() for p in retention.parameters() if p.requires_grad),
        )
        ...
        s_n_1s = [
            torch.zeros(hidden_size // heads, retention.v_dim // heads)
            .unsqueeze(0)
        ...
        Y_chunkwise = []
        for i in range(sequence_length // chunk_size):
            y_i, r_i = retention.forward_chunkwise(
                X[:, i * chunk_size : (i + 1) * chunk_size, :], r_n_1s, i
            )
            Y_chunkwise.append(y_i)
            r_n_1s = r_i
...
class TestRetNet(unittest.TestCase):
    ...
    def test_retnet(self):
        ...
        print(
            "Default v_dim:",
            sum(p.numel() for p in retnet.parameters() if p.requires_grad),
        )
        ...
        s_n_1s = [
            [
                torch.zeros(hidden_size // heads, retnet.v_dim // heads)
                .unsqueeze(0)
                .repeat(batch_size, 1, 1)
                for _ in range(heads)
            ]
