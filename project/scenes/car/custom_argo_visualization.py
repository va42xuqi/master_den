...
def plot_trajectory(path, input_list, output_true, output_pred):
    if not path.endswith(".parquet"):
        id = path.split("/")[-1]
        path += f"/scenario_{id}.parquet"
    path = Path(path)
    scenario_id = path.stem.split("_")[-1]
    static_map_path = (
            path.parents[0] / f"log_map_archive_{scenario_id}.json"
    )
    scenario = scenario_serialization.load_argoverse_scenario_parquet(path)
    static_map = ArgoverseStaticMap.from_json(static_map_path)
    viz_save_path = Path(f'{os.path.dirname(__file__)}/animation/cache/{scenario_id}.mp4')
    visualize_scenario(scenario, static_map, viz_save_path, input_list, output_true, output_pred)


def visualize_helper(scenario_static_map, ax, scenario, timestep, plot_bounds):
...
def visualize_scenario(scenario: ArgoverseScenario, scenario_static_map: ArgoverseStaticMap, save_path: Path,
                       input=None, output_true=None, output_pred=None) -> None:
    # Build each frame for the video
    frames: List[Image] = []
    plot_bounds: _PlotBounds = (0, 0, 0, 0)
    internal_fps_counter = 0
    list_counter = 0

    for timestep in range(_OBS_DURATION_TIMESTEPS + _PRED_DURATION_TIMESTEPS):
        print(f'\r[ArgoVis] Visualizing data ({scenario.scenario_id}): {((timestep+1) / (_OBS_DURATION_TIMESTEPS + _PRED_DURATION_TIMESTEPS) * 100):.2f}% ({timestep+1}/{_OBS_DURATION_TIMESTEPS + _PRED_DURATION_TIMESTEPS})',
              end="")
        sys.stdout.flush()
        for h in range(3 if (timestep % 2 == 0) else 2):
            _, ax = plt.subplots()

            # Create the predicted trajectory of the target
            if internal_fps_counter >= input[0].shape[2] and len(input) > list_counter:
                # Plot the input of all agents
                # Input of all agents
                _plot_polylines(input[list_counter][1:], color='grey', line_width=2)
                # Input of the target
                _plot_polylines(input[list_counter][:1], color='orange', line_width=2)
                # Input of the predicted output of the target
                _plot_polylines(output_pred[list_counter], color='red', line_width=2)
                # Input of the true output of the target
                _plot_polylines(output_true[list_counter], color='green', line_width=1)
                list_counter += 1

            buf = visualize_helper(scenario_static_map, ax, scenario, timestep, plot_bounds)
            plt.close()
            buf.seek(0)
            frame = img.open(buf)
            frames.append(frame)
            internal_fps_counter += 1


# Write buffered frames to MP4V-encoded video
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    vid_path = str(save_path.parents[0] / f"{save_path.stem}.mp4")
    video = cv2.VideoWriter(vid_path, fourcc, fps=25, frameSize=(1920, 1080))
    for i in range(len(frames)):
        frame_temp = np.array(frames[i].copy())
        frame_temp = cv2.resize(frame_temp, (1920, 1080))
...
def _plot_actor_tracks(ax: plt.Axes, scenario: ArgoverseScenario, timestep: int) -> Optional[_PlotBounds]:
    ...
    for track in scenario.tracks:
        ...
        elif track.track_id == 0 or track.track_id == "AV":
...
def _plot_polylines(
    ...
    if type(polylines) != list:
        for line in polylines:
            plt.plot(line[0, :], line[1, :], style, linewidth=line_width, color=color, alpha=alpha)
    else:
