...
csv_output_directory = "results/pretrained"
# Directory to save the LaTeX files
latex_output_directory = "tables/pretrained"
...
def process_files(scene_type, i):
    ...
    for root, dirs, files in os.walk(directory):
        for filename in files:
            if filename.endswith(".csv"):
                if f"_{i}_" in filename and scene_type in filename and "pretrained" in filename:
                    if not any(
                        keyword in filename
                        for keyword in ["pos", "vel", "finetuned", "uni", "pos", "vel"]
                    ):
    ...
    csv_file_path_1 = os.path.join(
        csv_output_directory, f"pivoted_data_{i}_{scene_type}_pretrained.csv"
    )
    ...
    latex_table_1 = pivot_df.to_latex(
        ...
        caption=f"Results for {scene_type} on pretrained with 1 player.",
        label=f"pre:{scene_type}",
    ...
    latex_file_path_1 = os.path.join(
        latex_output_directory, f"pivoted_data_{i}_{scene_type}_pretrained.tex"
    )
