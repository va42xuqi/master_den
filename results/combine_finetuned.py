...
csv_output_directory = "results/finetuned"
# Directory to save the LaTeX files
latex_output_directory = "tables/finetuned"
...
def process_files(scene_type, i):
    ...
    for root, dirs, files in os.walk(directory):
        for filename in files:
            if filename.endswith(".csv"):
                if (
                    f"_{i}_" in filename
                    and scene_type in filename
                    and "finetuned" in filename
                ):
                    if not any(
                        keyword in filename
                        for keyword in ["pos", "vel", "pretrained", "uni", "pos", "vel"]
                    ):
    ...
    for file_path in relevant_files:
        ...
        if "lstm" in model_name:
            model_name = "LSTM"
        if "1l_linear" in model_name or "one_layer_linear" in model_name:
            model_name = "one layer linear"
        if "2l_linear" in model_name or "two_layer_linear" in model_name:
    ...
    # Filter to keep only the specified metrics
    ...
    for metric in rad_metrics:
        if metric in pivot_df["Metric"].values:
            for col in pivot_df.columns[1:]:  # Skip the 'Metric' column
                pivot_df.loc[pivot_df["Metric"] == metric, col] = pivot_df.loc[
                    pivot_df["Metric"] == metric, col
                ].apply(convert_rad_to_deg)
    ...
    csv_file_path_1 = os.path.join(
        csv_output_directory, f"pivoted_data_{i}_{scene_type}_finetuned.csv"
    )
    ...
    latex_table_1 = pivot_df.to_latex(
        index=False,
        float_format="%.2f",
        column_format="l|" + "|c" * (pivot_df.shape[1] - 1),
        caption=f"Results finetuned on {scene_type} with 1 player.",
        label=f"fine:{scene_type}",
        escape=False,  # To handle special characters if needed
    )

    latex_table_1 = latex_table_1.replace(
        "\\begin{table}", "\\begin{table}[H]\n\\centering"
    )

    # Customize LaTeX table to remove top, mid, and bottom rules
    latex_table_1 = latex_table_1.replace("\\toprule", "")
    latex_table_1 = latex_table_1.replace("\\midrule", "\hline" * 2)
    latex_table_1 = latex_table_1.replace("\\bottomrule", "")
    # Save each LaTeX table to a separate .tex file
    latex_file_path_1 = os.path.join(
        latex_output_directory, f"pivoted_data_{i}_{scene_type}_finetuned.tex"
    )
