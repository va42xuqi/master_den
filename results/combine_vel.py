...
csv_output_directory = "results/vel"
# Directory to save the LaTeX files
latex_output_directory = "tables/vel"
    ...
    for root, dirs, files in os.walk(directory):
        for filename in files:
            if filename.endswith(".csv"):
                if "50" in filename and scene_type in filename and "vel" in filename:
                    if not any(
                        keyword in filename
                        for keyword in ["pos", "uni", "finetuned", "pretrained", "other"]
                    ):
