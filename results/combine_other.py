...
csv_output_directory = "results/inter"
# Directory to save the LaTeX files
latex_output_directory = "tables/inter"
...
def process_files(scene_type, i):
    ...
    for root, dirs, files in os.walk(directory):
        for filename in files:
            if filename.endswith(".csv"):
                if f"_{i}_" in filename and scene_type in filename and "other_team" in filename:
                    if not any(
                        keyword in filename
                        for keyword in ["pos", "vel", "finetuned", "pretrained"]
                    ):
    ...
    latex_table_1 = pivot_df.to_latex(
        ...
        caption=f"Results for {scene_type}, tested on other team.",
        label=f"other_team:{scene_type}",
