...
def process_files(scene_type):
    ...
    for root, dirs, files in os.walk(directory):
        for filename in files:
            if filename.endswith(".csv"):
                if "50" in filename and scene_type in filename and "uni" in filename:
                    if not any(
                        keyword in filename
                        for keyword in ["pos", "vel", "finetuned", "pretrained", "other"]
                    ):
    ...
    for file_path in relevant_files:
        ...
        if "bitnet" in model_name:
            model_name = "BitNet"
        if model_name == "one layer linear":
            model_name = "1L Linear"
        if model_name == "two layer linear":
