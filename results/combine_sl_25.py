...
csv_output_directory = "results/sl_25"
# Directory to save the LaTeX files
latex_output_directory = "tables/sl_25"
...
def process_files(scene_type):
    ...
    for root, dirs, files in os.walk(directory):
        for filename in files:
            if filename.endswith(".csv"):
                if f"_{sl_length}_" in filename and scene_type in filename:
                    if not any(
                        keyword in filename
                        for keyword in ["pos", "vel", "uni", "finetuned", "pretrained", "other"]
                    ):
    ...
    for file_path in relevant_files:
        ...
        if "bitnet" in model_name:
            model_name = "BitNet"
        if "1l" in model_name or "one layer" in model_name:
            model_name = "1L Linear"
        if "2l" in model_name or "two layer" in model_name:
    ...
    latex_file_path = os.path.join(latex_output_directory, f"pivoted_data_{sl_length}_{scene_type}.tex")
