...
from badass_trajectory_predictor.models.lmu.lmu_utils.lmu_layer import LMUFFT
...
class TargetLMU(nn.Module):
    ...
    def forward(self, x, hidden):
        lmu_out, hidden = self.lmu(x.unsqueeze(1), hidden)
        return self.linear(lmu_out[:, -1]), hidden
...
class SupLMU(Model):
    ...
    def forward(self, data, target=None, teacher_forcing=True):
        ...
        for t in range(self.pred_len):
            output, h_0 = self.target_lmu(input_seq, h_0)
            outputs[:, t, :] = output
