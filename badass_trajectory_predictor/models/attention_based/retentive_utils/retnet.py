...
from badass_trajectory_predictor.models.attention_based.retentive_utils.retention import (
    MultiScaleRetention,
)
...
class RetNet(nn.Module):
    ...
    def forward_recurrent(self, x_n, s_n_1s, n):
        ...
        for i in range(self.layers):
            ...
            o_n, s_n = self.retentions[i].forward_recurrent(
                self.layer_norms_1[i](x_n), s_n_1s[i], n
            )
            y_n = o_n + x_n
            s_ns.append(s_n)
    ...
    def forward_chunkwise(self, x_i, r_i_1s, i):
        ...
        for j in range(self.layers):
            o_i, r_i = self.retentions[j].forward_chunkwise(
                self.layer_norms_1[j](x_i), r_i_1s[j], i
            )
            y_i = o_i + x_i
            r_is.append(r_i)
...
if __name__ == "__main__":
    ...
    retnet = RetNet(layers, hidden_dim, ffn_size, heads, double_v_dim=True).to(device)
